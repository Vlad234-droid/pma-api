...
public class ContentEntryDAOTest extends AbstractDAOTest {
    ...
    void deleteTest(){
        ...
        assertEquals(0, contentEntryDAO.findByKey("knowledge-library/gb/content").size());
    }

    @Test
    @DataSet({BASE_PATH_TO_DATA_SET + "contents.xml"})
    void findByKeyTest() {
        assertEquals(1, contentEntryDAO.findByKey("knowledge-library/gb/content").size());
    }
    ...
    void updateTest() {
        ...
        assertEquals(ContentStatus.UNPUBLISHED, contentEntryDAO.findById(UUID.fromString(TEST_CONTENT_UUID)).getStatus());
    }
